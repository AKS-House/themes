...
<div class="pmagic">  
 ...
 <?php if(isset($pm_error) && $pm_error!='' && !is_user_logged_in()):?>
   <div class="pm-login-box-error pm-dbfl pm-pad10 pm-border-bt">
     <?php echo wp_kses_post($pm_error);?>
   </div>
 <?php endif;?>
...
?> 
  ...
  <div class="pm-login-header-buttons pm-dbfl pm-pad10">
      <div class="pm-center-button pm-difl pm-pad10">
      ...
      <div class="pm-center-button pm-difl pm-pad10">
        <a href="<?php echo esc_url(get_permalink()); ?>/?action=logout&_wpnonce=<?php echo esc_attr( wp_create_nonce( 'pg-logout' ) ); ?>" class="pm_button">
...
<form class="pmagic-form pm-dbfl pm-bg-lt" method="post" action="" id="pm_login_form" name="pm_login_form">
  ...
  <input type="text" name="<?php echo esc_attr('user_login');?>" id="<?php echo esc_attr('user_login');?>" 
         placeholder="<?php esc_attr_e('Email or Username','profilegrid-user-profiles-groups-and-communities');?>" required="required">
  
  <input type="password" name="<?php echo esc_attr('user_pass');?>" id="<?php echo esc_attr('user_pass');?>" 
         placeholder="<?php esc_attr_e('Password','profilegrid-user-profiles-groups-and-communities');?>" required="required">
  ...
  <div class="pm-login-box-bottom-container pm-dbfl pm-bg pm-border">
      <input type="submit" value="<?php esc_attr_e('Login','profilegrid-user-profiles-groups-and-communities');?>" 
             name="login_form_submit" class="pm-difl">
      
      <?php if($register_link):?>
        <a href="<?php echo esc_url($registration_url);?>" class="pm-difl pg-registration-button">
          <?php esc_html_e('Register','profilegrid-user-profiles-groups-and-communities');?> 
        </a> 
      <?php endif; ?>

      <div class="cf-turnstile" data-sitekey="0x4AAAAAAAxcgHRKyzYVRVTt"></div>
      
      <div class="pm-login-links-box pm-difr pm-pad10">
        <a href="<?php echo esc_url($forget_password_url);?>">
          <?php esc_html_e('Forgot Password?','profilegrid-user-profiles-groups-and-communities');?>
        </a>
      </div>
  </div>
</form>
